    ...
    <div class="card">
        <div class="card-header">
            <h5 class="fw-bold"><a class="text-dark"
                    href="{{ route('admin.contest.show', $contest->id) }}">{{ $contest->title }} </a> Votes</h5>
                                ...
                                <td data-label="@lang('Participant')">
                                    @if ($item->submission)
                                        <a href="{{ route('admin.submission.show', $item->submission_id) }}"
                                            class="text-primary text-wrap">{{ $item->submission->title ?? 'n/a' }}</a>
                                    @else
        ...
        @if ($votes->hasPages())
            <div class="card-footer text-end">{{ $votes->links() }}</div>
        @endif
    </div>
