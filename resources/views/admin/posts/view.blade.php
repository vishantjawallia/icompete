        ...
        <div class="d-flex justify-content-between mb-2">
            <div class="d-flex align-items-center py-2">
                <div class="d-inline-block position-relative">
                    <img src="{{my_asset($post->user->image)}}" alt="" class="rounded avatar avatar-md style-1">
                </div>
                <div class="clearfix ms-2">
                    <h6 class="mb-0 fw-semibold">{{$post->user->full_name}}</h6>
                    <span class="fs-13">{{show_datetime($post->created_at)}}</span>
                </div>
            </div>
            <div class="clearfix ms-auto">
                <div class="dropdown">
                    <button type="button" class="btn btn-secondary btn-icon-xxs tp-btn fs-18 border border-primary" data-bs-toggle="dropdown" aria-expanded="false">
                        <i class="bi bi-three-dots-vertical"></i>
                    </button>
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#editPostModal"><i class="bi bi-pencil-square me-2"></i>Edit</a></li>
                        <li><a class="dropdown-item text-danger delete-btn" href="{{ route('admin.community.delete', $post->id) }}"><i class="bi bi-trash me-2"></i>Delete</a></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="clearfix text-black">
            <p class="fs-14 mb-2">{{$post->content}}</p>
        </div>
...
<div class="modal fade" id="editPostModal" tabindex="-1" aria-labelledby="editPostModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.community.update', $post->id) }}" class="ajaxForm" method="POST">
                @csrf
                @method('PUT')
...
<div class="card">
    ...
    <div class="card-body">
        <div id="comments-container">
            @forelse ($post->comments()->orderByDesc('updated_at')->get() as $comment)
